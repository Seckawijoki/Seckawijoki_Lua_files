...
function AbsLayoutManager:get(i)
	return i >= 1 and i <= #self.m_aUIObjects and self.m_aUIObjects[i] or nil;
end
...
--[==[
	设置目标UI的尺寸	
	Created on 2019-6-18 at 21:08:52
]==]
function AbsLayoutManager:resetOtherPlaneWithMinimalSize(szOtherUIName, iMinWidth, iMinHeight)
	if not self.m_bHasCalculatedPlaneSize then 
		self:calculatePlaneSize();
	end
	if not szOtherUIName then return self end
	...
	iMinWidth = iMinWidth > self.m_iPlaneWidth and iMinWidth or self.m_iPlaneWidth
	iMinHeight = iMinHeight > self.m_iPlaneHeight and iMinHeight or self.m_iPlaneHeight
	uiOther:SetWidth(iMinWidth);
	uiOther:SetHeight(iMinHeight);
	return self;
end
...
function AbsLayoutManager:findIrregularUI(szUIName)
	if not szUIName then return self end
	local ui = getglobal(szUIName)
	if not ui then return self end
	for i = 1,#self.m_aUIObjects do
		if self.m_aUIObjects[i] == ui then
			return self,self.m_aUIObjects[i]
		end
	end
end
...
function AbsLayoutManager:removeIrregularUI(szUIName)
	if not szUIName then return self end
	local ui = getglobal(szUIName)
	if not ui then return self end
	for i = 1,#self.m_aUIObjects do
		if self.m_aUIObjects[i] == ui then
			table.remove(self.m_aUIObjects,i)
			break;
		end
	end
	return self
...
function AbsLayoutManager:removeAll()
	self.m_aUIObjects = {}
	return self
end
...
function AbsLayoutManager:resetUIFontSize(iFontSize)
	...
	for i=1, length do 
		if aUIObjects[i].SetFontSize then
...
---------------------------------------------------------------AbsGridLayoutManager start---------------------------------------------------------------
local AbsGridLayoutManager = {

}
ClassesCache:insertSuperClass(AbsGridLayoutManager, AbsLayoutManager)

--[==[
	重写时，调用AbsLayoutManager的onRecycle
	Created on 2021-03-01 at 11:47:36
]==]
function AbsGridLayoutManager:onRecycle()
	local AbsGridLayoutManager = self.super
	AbsGridLayoutManager.super.onRecycle(self);
	self.m_bHorizontal = true;
	return self;
end
...
function AbsGridLayoutManager:calculatePlaneSize()
	-- self.m_iMaxColumn = iMaxColumn > 0 and iMaxColumn or self.m_iMaxColumn;
	...
	-- print("AbsGridLayoutManager:calculatePlaneSize(): iTargetWidth = ", iTargetWidth, "iTargetHeight = ", iTargetHeight);
...
function AbsGridLayoutManager:onLayoutGrid(uiTarget, iCurrentRow, iCurrentColumn)
	-- print("AbsGridLayoutManager:onLayoutGrid(): iCurrentRow = ", iCurrentRow, "iCurrentColumn = ", iCurrentColumn);
	...
	-- print("AbsGridLayoutManager:onLayoutGrid(): totalOffsetX = ", totalOffsetX, "totalOffsetY = ", totalOffsetY);
	uiTarget:SetPoint(
		self.m_szPoint,
		self.m_szRelativeTo,
...
function AbsGridLayoutManager:onLayoutIndex(index)
...
function AbsGridLayoutManager:layoutAll(iCount)
	self.m_iCount = iCount;
	self.m_iLayoutEndIndex = iCount
	-- print("AbsGridLayoutManager:layoutAll(): iCount = ", iCount);
	for i=1, iCount do 
...
---------------------------------------------------------------AbsGridLayoutManager end---------------------------------------------------------------
---------------------------------------------------------------AbsHorizontalSplitLayoutManager start---------------------------------------------------------------
local AbsHorizontalSplitLayoutManager = {

}
ClassesCache:insertSuperClass(AbsHorizontalSplitLayoutManager, AbsLayoutManager);
--[==[
	@Override
	@param iMaxVisibleCount 最大可见控件数量
	@param iMaxCount XML分配的最大控件数量
]==]
function AbsHorizontalSplitLayoutManager:layoutAll(iMaxVisibleCount, iMaxCount)
...
---------------------------------------------------------------AbsHorizontalSplitLayoutManager end---------------------------------------------------------------
---------------------------------------------------------------AbsRelativeOffsetGridLayoutManager start---------------------------------------------------------------
local AbsRelativeOffsetGridLayoutManager = {

}
ClassesCache:insertSuperClass(AbsRelativeOffsetGridLayoutManager, AbsLayoutManager)

--[==[
	@Override
]==]
function AbsRelativeOffsetGridLayoutManager:layoutAll(iCount)
...
function AbsRelativeOffsetGridLayoutManager:calculatePlaneSize()
	-- local bDebug = self.m_bDebug;
	local math = _G.math
...
---------------------------------------------------------------AbsRelativeOffsetGridLayoutManager end---------------------------------------------------------------
---------------------------------------------------------------AbsLinearLayoutManager start---------------------------------------------------------------
local AbsLinearLayoutManager = {

}
ClassesCache:insertSuperClass(AbsLinearLayoutManager, AbsLayoutManager);

function AbsLinearLayoutManager:onRecycle()
	local AbsLinearLayoutManager = self.super;
	AbsLinearLayoutManager.super.onRecycle(self);
	self.m_szPoint = "center";
	return self;
end
...
function AbsLinearLayoutManager:layoutAll()
	local aUIObjects = self.m_aUIObjects;
	local length = #aUIObjects;
	if length <= 0 then
		return self;
...
function AbsLinearLayoutManager:calculatePlaneSize()
...
function LayoutManagerFactory:newGridLayoutManager()
	return ClassesCache:obtain("GridLayoutManager", AbsGridLayoutManager)
end

--[==[
	从指定UI水平向两边平均分配批量子控件
]==]
function LayoutManagerFactory:newHorizontalSplitLayoutManager()
	return ClassesCache:obtain("HorizontalSplitLayoutManager", AbsHorizontalSplitLayoutManager)
end

--[==[
	相邻UI相对偏移分布
]==]
function LayoutManagerFactory:newRelativeOffsetGridLayoutManager()
	return ClassesCache:obtain("RelativeOffsetGridLayoutManager", AbsRelativeOffsetGridLayoutManager)
end
...
function LayoutManagerFactory:newLinearLayoutManager()
	return ClassesCache:obtain("LinearLayoutManager", AbsLinearLayoutManager)
end
